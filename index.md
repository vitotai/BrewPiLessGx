...
- Customized display. Display information you need.
    - Two built-in skins available. one for simple BrewPi information, and the other one has all available information.
    - It can be configured to show only hydrometer information, thus used as a iSpindel, Pill ,or Tilt reapeater.

# Screen
There are three UI screens
...
# Custom Hardware
There are so many difference hardware configurations that I can't make it to support all of them. Even with the same driver IC, there might be different configurations. Without verification with the real devices, I won't declare that it is supported. The adpation is not diffucult if you can find the drivers for your hardware.
...
# Operation
The main display is customizable, defined by a JSON file. The syntax and format is described in the [document](doc/skin.md). There is a not-so-good UI designer in (extra/SkinEditor)[https://vitotai.github.io/BrewPiLessGx/extra/SkinEditor/index.htm].
