...
typedef std::function<void(const char* ssid,int32_t rssi, bool enc, uint16_t index, uint16_t total)> NetScanResultCb;
...
public:
	WiFiSetupClass():_wifiState(WiFiStateUnknown),_wifiScanState(WiFiScanStateNone),_switchToAp(true),_autoReconnect(true),
		 _maxReconnect(5),_netScanResultCb(NULL),_eventHandler(NULL),_targetSSID(NULL),_targetPass(NULL),_ip(INADDR_NONE),_gw(INADDR_NONE),_nm(INADDR_NONE){}
	...
	bool requestScanWifi(NetScanResultCb scanResultCb=NULL);
...
private:
	...
	unsigned long _time;
	NetScanResultCb _netScanResultCb;
