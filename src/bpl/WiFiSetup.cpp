...
bool WiFiSetupClass::requestScanWifi(NetScanResultCb scanResultCb) {
	if(_wifiScanState == WiFiScanStateNone){
		_wifiScanState = WiFiScanStatePending;
		_netScanResultCb = scanResultCb;
		return true;
	}
	return false;
}

void WiFiSetupClass::_scanNetworks(void) {
	bool apmode=false;
    ...
    DBG_PRINTF("Scan done, found %d\n",n);
    if (n <= 0) {
    	DBG_PRINTF("No networks found");
		if(_netScanResultCb){
			_netScanResultCb(NULL,0,true,0,0);
		}
		...
		if(_netScanResultCb){
			...
			for (int i = 0; i < n; i++) {
				...
				DBG_PRINTF("%d, entry:%s\n",idx,WiFi.SSID(indices[i]).c_str());
				_netScanResultCb( WiFi.SSID(indices[i]).c_str(),WiFi.RSSI(indices[i]), WiFi.encryptionType(indices[i]) != WIFI_AUTH_OPEN,
					idx, total);
				idx ++;
			}
			...
			if(_eventHandler) _eventHandler(rst.c_str());
		}
	}
