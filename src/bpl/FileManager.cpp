...
#define SPIFFS_FORMAT_PATH     "/format-filesystem"
#define SPIFFS_FORMATTING_PATH "/exeformat-spiffs"

static const char PROGMEM filesystem_format_html[]  =   { 0x1f, 0x8b, 0x08, 0x08, 0x17, 0xcf, 0x09, 0x5a, 0x02, 0x03, 0x66, 0x6f,
...
static const char PROGMEM filesystem_formating_html[]  =  {
...
void FileManager::handleRequest(AsyncWebServerRequest *request){
    ...
    if(request->method() == HTTP_GET){
        if(request->url().equals("/list")) _handleFileList(request);
            ...
            AsyncWebServerResponse *response = request->beginResponse_P(200, "text/html", (const uint8_t*)filesystem_format_html, sizeof(filesystem_format_html));
            response->addHeader("Content-Encoding", "gzip");
            request->send(response);
        }else if(request->url().equals(SPIFFS_FORMATTING_PATH)){
            AsyncWebServerResponse *response = request->beginResponse_P(200, "text/html", (const uint8_t*)filesystem_formating_html, sizeof(filesystem_formating_html));
            response->addHeader("Content-Encoding", "gzip");
            request->send(response);
...
//     s        DBG_PRINTF("canHandle: %s comapre to %s\n",_path.c_str(),request->url().c_str());
        ...
        else if(request->url().equals(SPIFFS_FORMAT_PATH)) return true;
        else if(request->url().equals(SPIFFS_FORMATTING_PATH)) return true;
    }else if(request->url().equalsIgnoreCase("/edit")){
